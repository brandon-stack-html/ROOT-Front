  ...
  var Auth = {
    ...
    _renderNotifDropdown: function (dd) {
      ...
      var rows = notifs.map(function (n) {
        return '<a href="' + BASE + n.href + '" onclick="Auth._markRead(\'' + n.id + '\');document.getElementById(\'notifDropdown\').style.display=\'none\'" style="display:flex;align-items:flex-start;gap:10px;padding:11px 16px;text-decoration:none;background:' + (n.leida ? 'transparent' : 'color-mix(in srgb, var(--accent) 4%, transparent)') + ';border-bottom:1px solid var(--border-subtle,var(--border));transition:background .12s;">' +
          '<div style="width:28px;height:28px;border-radius:7px;background:' + (n.leida ? 'var(--alt)' : 'color-mix(in srgb, var(--accent) 10%, transparent)') + ';display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:1px;">' +
